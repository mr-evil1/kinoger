...
def _git_commit(fname):
    try:
        subprocess.run(['git', 'add', 'html/', 'data/'], check=False)
        result = subprocess.run(
            ['git', 'commit', '-m', 'scrape: %s [skip ci]' % fname],
            capture_output=True, text=True,
        )
        if result.returncode == 0:
            subprocess.run(['git', 'push'], check=False)
            print(f'  git: {fname} committed & pushed', flush=True)
    except Exception as e:
        print(f'  git FEHLER: {e}', flush=True)


def _fetch_and_save(ctx, slug, url, retries=3, delay=4, progress=None):
    if progress is None:
        progress = {}
    ...
    if page_num > 1:
        print(f'  {slug}: Fortsetze ab Seite {page_num}', flush=True)
    ...
    while current_url and current_url not in seen:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                ok   = _save_html(fname, html)
                if ok:
                    print(f'  {fname}: OK', flush=True)
                    _update_catalog(_parse_entries(html))
                    _git_commit(fname)
                    ...
                    break
                print(f'  {fname}: Block (Versuch {attempt}/{retries}), warte {delay * attempt}s...', flush=True)
    ...
    _save_progress(progress)
    print(f'  {slug}: komplett ({page_num - 1} Seiten)', flush=True)
...
def scrape():
    ...
    with sync_playwright() as p:
        ...
        for key, url in PAGES.items():
            if progress.get(key, {}).get('done'):
                print(f'  {key}: bereits fertig, überspringe.', flush=True)
                continue
            _fetch_and_save(ctx, key, url, progress=progress)
        ...
        for genre in genres:
            ...
            if progress.get(slug, {}).get('done'):
                ...
                continue
            _fetch_and_save(ctx, slug, genre['url'], progress=progress)
            time.sleep(PAGE_DELAY)

        browser.close()

    print('Baue catalog.json aus allen HTML-Dateien...', flush=True)
    for fname in os.listdir(HTML_DIR):
        if not fname.endswith('.html') or fname == 'home.html':
            continue
    ...
    print('catalog.json fertig.', flush=True)
    print('HTML-Generierung abgeschlossen.', flush=True)
