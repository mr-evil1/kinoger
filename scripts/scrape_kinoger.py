...
def _fetch_and_save(ctx, slug, url, retries=3, delay=4):
    print(f'  {slug}: {url}', flush=True)
    for attempt in range(1, retries + 1):
        try:
            html = _fetch_page(ctx, url)
            ok = _save_html(slug, html)
            if ok:
                print(f'  {slug}: OK', flush=True)
                return
            print(f'  {slug}: Block (Versuch {attempt}/{retries}), warte {delay * attempt}s...', flush=True)
            time.sleep(delay * attempt)
        except Exception as e:
            print(f'  {slug} FEHLER (Versuch {attempt}/{retries}): {e}', flush=True)
            time.sleep(delay * attempt)
    print(f'  {slug}: aufgegeben nach {retries} Versuchen', flush=True)
...
def scrape():
    ...
    with sync_playwright() as p:
        ...
        for key, url in PAGES.items():
            _fetch_and_save(ctx, key, url)

        print(f'Lade Genre-Seiten ({len(genres)})...', flush=True)
        for genre in genres:
            _fetch_and_save(ctx, _slug(genre['title']), genre['url'])
            time.sleep(2)
